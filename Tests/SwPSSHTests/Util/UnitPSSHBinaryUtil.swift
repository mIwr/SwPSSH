...
class UnitPSSHBinaryUtil: XCTestCase {
    
    func testConvertToInt16() {
        let buff: [UInt8] = [0x10, 0x11, 0x12, 0x13]
        
        let converted: Int16? = PSSHBinaryUtil.getVal(buff, offset: 1, bigEndian: true)
    ...
    func testConvertToInt64() {
        ...
        let converted: Int64? = PSSHBinaryUtil.getVal(buff, bigEndian: true)
    ...
    func testGetBytesFromInt32() {
        ...
        let buff: [UInt8] = [0x10, 0x11, 0x12, 0x13]
        let bytes = PSSHBinaryUtil.getBytes(val)
